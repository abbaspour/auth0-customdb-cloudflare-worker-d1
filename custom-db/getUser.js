async function getUser(identifierValue, context, callback) {
    console.log(`getUser: ${identifierValue}, context: ${JSON.stringify(context)}`);
    ...
    console.log(`found user with ID: ${user_id}, identifierType: ${identifierType}`);

    if (!user_id) {
        return callback(null);
    }

    const profile = {'email': identifierValue, user_id: `${user_id}`};
